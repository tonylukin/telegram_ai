...
async def promote_users(super_admin_bot_client: BotClient, promoting_bot_client: BotClient, group_username: str) -> bool:
...
async def main():
    ...
    for channel in OWN_CHANNELS:
        for bot_client in all_bots_clients:
            try:
                result = await promote_users(super_admin_bot_clients[0], bot_client, channel)
