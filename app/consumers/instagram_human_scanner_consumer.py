...
from app.config import \
    APP_HOST, API_TOKEN, RABBITMQ_QUEUE_INSTAGRAM_HUMAN_SCANNER
...
class InstagramHumanScannerConsumer(HumanScannerConsumer):
    ...
    async def _get_desc_from_api(self, payload: dict[str, str], lang_code: str) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{APP_HOST}/user-info/ig-collect", json=payload, headers=headers) as resp:
                if resp.status == 200:
    ...
    def get_notification_message(self) -> str | None:
        ...
        return f"[Instagram] Username: {payload['username']}\nDescription: {self.output_data.get('desc')}"

async def main():
    consumer = InstagramHumanScannerConsumer(queue=RABBITMQ_QUEUE_INSTAGRAM_HUMAN_SCANNER, notification_sender=NotificationSender())
    await consumer.init()
